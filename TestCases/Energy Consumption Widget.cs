...
{
    public class Energy_consumption_Widget : BaseClass
    {
       
        static string weburl_runsettings = TestContext.Parameters["weburl"].ToString();
        public void TotalEnergyConsumptionInformation(Xpath__Fuel_Consumption Xpath, Values__Energy_Usage Value, Filter_Query_Params Filters)
        {
            try
            {
                ...
                //Scroll website
                Scroll(Xpath.EnergyConsumption);
                wait.Until(ExpectedConditions.ElementExists(By.XPath(Xpath.EnergyConsumption)));
                var EnergyConsumptionWidgetPosition = driver.FindElement(By.XPath(Xpath.EnergyConsumption));
                js.ExecuteScript("arguments[0].scrollIntoView(true);", EnergyConsumptionWidgetPosition);

                // Verify Fuel consumption label
                AssertElementIsEqualTo(Xpath.EnergyConsumption, "Energy consumption");

                // Fleet KPI
                wait.Until(ExpectedConditions.ElementExists(By.CssSelector(Xpath.AverageLabelFleet)));
                AssertElementIsEqualTo(Xpath.EnergyConsumptionValue, Value.AverageFleet);
            }
