...
{
    public class Fuel_consumption_Widget: BaseClass
    {
       
        static string weburl_runsettings = TestContext.Parameters["weburl"].ToString();
        public void TotalFuelConsumptionInformation(Xpath__Fuel_Consumption Xpath, Values__Fuel_Consumption Value, Filter_Query_Params Filters)
        {
            try
            { 
                ...
                //Scroll website
                Scroll(Xpath.FuelConsumption);
                wait.Until(ExpectedConditions.ElementExists(By.XPath(Xpath.FuelConsumption)));
                var FuelConsumptionwidgetPosition = driver.FindElement(By.XPath(Xpath.FuelConsumption));
                js.ExecuteScript("arguments[0].scrollIntoView(true);", FuelConsumptionwidgetPosition);

                // Verify Fuel consumption label
                AssertElementIsEqualTo(Xpath.FuelConsumption, "Fuel consumption");

                // Fleet KPI
                wait.Until(ExpectedConditions.ElementExists(By.CssSelector(Xpath.AverageLabelFleet)));
                AssertElementIsEqualTo(Xpath.AverageFleetValue, Value.AverageFleet);
            }
